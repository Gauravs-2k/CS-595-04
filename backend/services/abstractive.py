...
def _strip_date_suffix(text: str) -> str:
    """Strip trailing date like ': 01/01/1981' or ': 12/12/2023' from AH entity names."""
    import re
    return re.sub(r":\s*\d{2}/\d{2}/\d{4}\s*$", "", text).strip()


def _normalize_dx_key(text: str) -> str:
    """Normalize diagnosis text for dedup: strip ICD/SNOMED codes and whitespace."""
    import re
    # Remove bracketed codes like [Z30.09 ICD-10]
    text = re.sub(r"\[.*?\]", "", text).strip()
    # Remove standalone ICD-10 codes (e.g. Z30.09, Z68.26)
    text = re.sub(r"\b[A-Z]\d{2}(\.\d{1,4})?\b", "", text).strip()
    # Collapse whitespace
    text = re.sub(r"\s+", " ", text)
    return text.lower().strip()


def _notes_to_pcp_chart(notes: list[dict]) -> ClinicalDocument:
    """Treat ALL notes as PCP history (the 'before' data), deduplicating entities."""
    if not notes:
        return ClinicalDocument(raw_text="No records retrieved.")

    all_diagnoses, all_meds, all_labs, raw_parts = [], [], [], []
    seen_dx, seen_meds, seen_labs = set(), set(), set()

    for n in notes:
        sec = n.get("section_content", {})
        entities = _section_to_entities(sec)
        for d in entities.diagnoses:
            key = _normalize_dx_key(d.text)
            if key not in seen_dx:
                seen_dx.add(key)
                all_diagnoses.append(d)
        for m in entities.medications:
            key = _strip_date_suffix(m.name).lower().strip()
            if key not in seen_meds:
                seen_meds.add(key)
                # Store the cleaned name (without date)
                m.name = _strip_date_suffix(m.name)
                all_meds.append(m)
        for l in entities.labs:
            key = _strip_date_suffix(l.name).lower().strip()
            if key not in seen_labs:
                seen_labs.add(key)
                l.name = _strip_date_suffix(l.name)
                all_labs.append(l)
        raw_parts.append(_section_to_raw(sec))

    return ClinicalDocument(
        raw_text="\n".join(raw_parts),
        structured=ClinicalEntities(diagnoses=all_diagnoses, medications=all_meds, labs=all_labs),
    )


def _parse_zip_pcp_only(zip_bytes: bytes, patient_id: str) -> ClinicalDocument:
    notes = []
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
        ...
        logger.info("ah: found %d note files for patient_id=%s (pcp-only)", len(note_files), patient_id)
    ...
    return _notes_to_pcp_chart(notes)
...
class AbstractiveClient:
    ...
    async def search_patient(self, patient_data: dict) -> list[dict]:
        ...
        try:
            ...
            if response.status_code not in (200, 202):
                raise Exception(f"AH returned status {response.status_code}")

            data = response.json()

            if data.get("status") == "failure":
                raise Exception(f"AH search failure: {data.get('failure_reason', 'unknown')}")
            ...
            logger.warning("ah: live search failed (%s), falling back to mock results", exc)
            return mock_abstractive.search_patient(patient_data=patient_data)

    async def retrieve_records(self, patient_id: str) -> PatientRecords:
        from services.mimic_loader import is_mimic_patient
        if _use_mock() or patient_id.startswith("demo-") or is_mimic_patient(patient_id):
            logger.info("abstractive.retrieve_records: mock/mimic mode patient_id=%s", patient_id)
    ...
    async def retrieve_pcp_chart(self, patient_id: str) -> ClinicalDocument:
        """Retrieve only the PCP chart (patient history / 'before' data)."""
        from services.mimic_loader import is_mimic_patient
        if _use_mock() or patient_id.startswith("demo-") or is_mimic_patient(patient_id):
            logger.info("abstractive.retrieve_pcp_chart: mock/mimic mode patient_id=%s", patient_id)
            return mock_abstractive.retrieve_pcp_chart(patient_id=patient_id)

        # Live AH mode: fetch records and treat ALL notes as PCP history
        meta = _patient_cache.get(patient_id)
        if not meta:
            raise HTTPException(status_code=400, detail=f"No search session found for patient_id={patient_id}. Search first.")
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            deadline = time.time() + 300
            while True:
                logger.info("ah: retrieve_patient_docs (pcp) patient_id=%s", patient_id)
                response = await client.post(f"{settings.ah_base_url}/retrieve-patient-docs", json=payload)
                ...
                docs = response.json()
                if docs.get("status") == "failure":
                    raise HTTPException(status_code=502, detail=f"AH retrieve failed: {docs.get('failure_reason', 'unknown')}")
                if docs.get("status") == "success" and not docs.get("processing", True):
                    break
                if time.time() > deadline:
                    raise HTTPException(status_code=504, detail="Timed out waiting for AH documents")

                await asyncio.sleep(10)

        if docs.get("status") != "success" or not docs.get("results"):
            raise HTTPException(status_code=502, detail="AH returned no documents")

        url = docs["results"][0].get("url")
        if not url:
            raise HTTPException(status_code=502, detail="AH document URL missing")

        async with httpx.AsyncClient(timeout=60.0) as dl_client:
            zip_response = await dl_client.get(url)
        zip_response.raise_for_status()

        return _parse_zip_pcp_only(zip_response.content, patient_id)

    def get_demo_handoff(self, patient_id: str) -> str | None:
        """Return demo handoff text for MIMIC patients (for upload page pre-fill)."""
        return mock_abstractive.get_demo_handoff_text(patient_id)
